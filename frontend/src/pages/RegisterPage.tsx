...
import { LayoutDashboard, Lock, User, ArrowRight, Loader2, ShieldCheck, AlertCircle } from 'lucide-react';
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-card border border-slate-100 p-8 w-full max-w-md animate-fade-in">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            >
                {isLoading ? <Loader2 className="animate-spin" size={20} /> : <>Daftar <ArrowRight size={18} /></>}
            </button>
        </form>

        <p className="text-center text-sm text-slate-500 mt-6">
            Sudah punya akun? <Link to="/login" className="text-primary-600 font-medium hover:underline">Masuk disini</Link>
        </p>
      </div>
    </div>
  );
