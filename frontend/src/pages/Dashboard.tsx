...
export default function Dashboard() {
  ...
  const recentItems = Array.isArray(recentDocs) ? recentDocs : (recentDocs?.items || recentDocs?.results || []);
    ...
    <div className="space-y-8 animate-fade-in max-w-full">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div className="text-right hidden md:block">
            <p className="text-sm font-medium text-slate-500 uppercase tracking-wider">{currentDate}</p>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
            <div className="bg-white rounded-xl shadow-card border border-slate-100 overflow-hidden">
                <div className="p-6 border-b border-slate-100 flex items-center justify-between">
                    ...
                    <Link to="/search" className="text-sm text-primary-600 font-medium hover:text-primary-700 hover:underline">Lihat Semua</Link>
                </div>
                <div className="overflow-x-auto">
                    <table className="w-full">
                        <thead className="bg-slate-50 border-b border-slate-100">
                            <tr>
                                <th className="text-left py-3 px-6 text-xs font-semibold text-slate-500 uppercase tracking-wider">Dokumen</th>
                                <th className="text-left py-3 px-6 text-xs font-semibold text-slate-500 uppercase tracking-wider">Jenis</th>
                                <th className="text-left py-3 px-6 text-xs font-semibold text-slate-500 uppercase tracking-wider">Tanggal</th>
                                <th className="text-right py-3 px-6 text-xs font-semibold text-slate-500 uppercase tracking-wider">Aksi</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100">
                            {recentItems.length === 0 ? (
                                <tr>
                                    <td colSpan={4} className="py-8 text-center text-slate-400 text-sm">Belum ada dokumen.</td>
                                </tr>
                            ) : (
                                recentItems.map((item: any) => (
                                    <tr key={item.id} className="hover:bg-slate-50 transition-colors group">
                                        <td className="py-4 px-6">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div className="max-w-[180px]">
                                                    <p className="text-sm font-medium text-slate-900 truncate" title={item.perihal || item.filename}>{item.perihal || "Tanpa Judul"}</p>
                                                    <p className="text-xs text-slate-500 truncate">{item.nomor_surat || "-"}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="py-4 px-6">
                                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium capitalize ${
                                                item.jenis === 'masuk' ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'
                                            }`}>
                                        ...
                                        <td className="py-4 px-6 text-right">
                                            <Link to={`/documents/${item.id}`} className="text-slate-400 hover:text-primary-600 p-1 rounded-full hover:bg-slate-100 inline-block">
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-card border border-slate-100 p-6">
                 ...
                 <div className="space-y-3">
                    <Link to="/upload" className="flex items-center gap-4 p-3 rounded-lg border border-slate-200 hover:border-primary-500 hover:bg-primary-50 transition-all group cursor-pointer block">
                    ...
                    <Link to="/search" className="flex items-center gap-4 p-3 rounded-lg border border-slate-200 hover:border-purple-500 hover:bg-purple-50 transition-all group cursor-pointer block">
            ...
            {/* System Status / Tips */}
             <div className="bg-slate-900 rounded-xl shadow-lg p-6 text-white relative overflow-hidden">
                <div className="relative z-10">
                    ...
                    <p className="text-slate-300 text-sm mb-4">Pastikan format penamaan file konsisten untuk mempermudah pencarian otomatis oleh sistem.</p>
                </div>
                {/* Decor */}
...
function StatCardV2({ label, value, trend, trendUp, icon, color }: { label: string, value: string | number, trend: string, trendUp: boolean, icon: React.ReactNode, color: string }) {
    return (
        <div className="bg-white rounded-xl p-6 shadow-card border border-slate-100 hover:shadow-card-hover transition-all duration-300">
            <div className="flex items-start justify-between">
                ...
                <div className={`p-3 rounded-lg ${color} shadow-lg shadow-primary-500/20`}>
                    {icon}
                </div>
            </div>
